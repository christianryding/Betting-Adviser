...
def get_ended_events():
    try:
        ...
        # save completed and current bets individually
        finished_bets = []
        bets_made = []
        ...
        with open("bets.csv", "r") as csv_bets:
            ...
            for line in csv_reader:
                ...
                for event_info in results:
                    ...
                    # TODO fix 0-0/draw problem
                    if (line[0] == event_info["id"] and event_info["time_status"] == "3" and event_info["ss"] != None and event_info["ss"] != "0-0" and event_info["ss"] != "1-1" and event_info["ss"] != "2-2" and event_info["ss"] != "3-3"):
                        finished_bets.append(BetsCompleted(
                                event_info["id"],
        ...
        with open("bets_summary.csv", "a+", newline="") as f:
            for bets in finished_bets:
                ...
                if bets.team_betted_on == "H" and event_result[0] > event_result[1]:
                    ...
                    logging.info(
                        "won on A {} vs {} with id {}, betted on {}, final score {}!".format(
                            bets.home_name,
                            bets.away_name,
                            bets.eventID,
                            bets.team_betted_on,
                    ...
                    logging.info("Lost on {}, {} vs {}, betted on {}, final score {}!".format(bets.eventID ,bets.home_name,bets.away_name,bets.team_betted_on,bets.final_score))
        ...
        for events_data in events_access:
            try:
                ...
                if (int(event_time) - int(current_epoch_time)) > 0 and (
                    int(event_time) - int(current_epoch_time)
                ) < 7200:
                    events_list.append(
                        Event(event_id, event_time, home_team, away_team, league)
                    )
